...
# ... (imports existentes y código de utilities_minipdf) ...
...
def main():
    st.set_page_config(
        page_title="MiniPDF - Herramientas PDF",
        page_icon="📄",
        layout="centered"
    )
    ...
    app_mode = st.sidebar.radio(
        ...
        ["Combinar/Comprimir PDF", "Separador CLC"],
        index=0
    )
    
    if app_mode == "Combinar/Comprimir PDF":
        show_compressor_module()
    else:
        show_clc_module()

def show_compressor_module():
    st.title("📄 MiniPDF | Combina, Separa y Comprime archivos PDF")
    
    # Configuración específica para el módulo de compresión
    ...
    # Área principal del módulo de compresión
...
def show_clc_module():
    ...
    st.info("Sube un PDF multipágina y un Excel con ID_PROYECTO y PAG para separar y renombrar automáticamente")
    
    # Subir archivos para el módulo CLC
    clc_pdf = st.file_uploader("Subir PDF para separar", type="pdf", key="clc_pdf")
    clc_excel = st.file_uploader("Subir archivo Excel (columnas: ID_PROYECTO, PAG)", type=["xlsx", "xls"], key="clc_excel")
    ...
    if clc_pdf and clc_excel:
        try:
            ...
            # Validar columnas
            if 'ID_PROYECTO' not in df.columns or 'PAG' not in df.columns:
                st.error("El archivo Excel debe contener las columnas 'ID_PROYECTO' y 'PAG'")
                return
